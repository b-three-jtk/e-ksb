...
const role = computed(() => {
    return page.props.auth?.role || null;
});

// Get photo URL from foto_profil
const photoUrl = computed(() => {
    if (user.value?.foto_profil) {
        return `/storage/${user.value.foto_profil}`;
    }
    return null;
});
...
<template>
    <nav class="sticky nav-bar font-manrope border-gray-200 px-10">
        ...
        >
            ...
            <!-- Right Section -->
            <div
                class="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse items-center gap-4"
            >
                <ThemeToggler />
                <!-- Guest Auth Section -->
                <template v-if="!user">
                ...
                <template v-else>
                    ...
                    <Link :href="page.url.startsWith('/user') ? '/user/dashboard' : (role === 'Anggota' ? '/user/dashboard' : '/admin/dashboard')"
                    ...
                    <div class="relative">
                        ...
                        >
                            ...
                            <div class="flex flex-col text-left">
                                ...
                                >
                                <span
                                    class="mt-0.5 block text-theme-xs text-accent"
                                >
                                    {{ user.kode_pengguna }}
                                </span>
                            </div>
                ...
                >
                    <li v-for="menu in menuItems" :key="menu.path">
                        <Link
                            ...
                            :class="
                                isActive(menu.path)
                                    ? 'block pb-2! px-3 text-gray-900 hover:bg-gray-100 font-semibold md:hover:bg-transparent md:p-0 dark:text-white border-b-accent border-b-2'
                                    : 'block pb-2! px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:p-0 dark:text-white'
                            "
                        >
                            {{ menu.name }}
                        </Link>
                ...
                >
                    <li v-for="menu in menuItems" :key="menu.path">
                        <Link
                            ...
                            :class="
                                isActive(menu.path)
                                    ? 'block px-3 py-2 text-brand-600 font-semibold bg-gray-100 rounded dark:bg-gray-700 dark:text-white'
                                    : 'block px-3 py-2 text-gray-900 rounded hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700'
                            "
                        >
                            {{ menu.name }}
                        </Link>
