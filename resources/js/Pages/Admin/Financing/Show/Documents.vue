...
const hasDocuments = computed(() => {
    return documentsList.some(doc => props.data?.documents?.[doc.key])
})
    ...
    <div class="card-layout flex flex-col h-fit gap-4">
        ...
        <div v-if="hasDocuments" class="flex flex-col gap-2">
            <a
                ...
                v-show="props.data?.documents?.[doc.key]"
                :key="doc.key"
                :href="props.data?.documents?.[doc.key]"
                target="_blank"
                rel="noopener noreferrer"
                @click.prevent="handleDownload(props.data?.documents?.[doc.key], doc.label)"
