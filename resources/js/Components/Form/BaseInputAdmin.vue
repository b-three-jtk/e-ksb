...
<template>
    <div>
        <label v-if="label" class="mb-1.5 flex items-center gap-1 text-sm font-medium text-gray-700 dark:text-gray-300">
            {{ label }}
                <Tooltip v-if="hint">
                    <p>{{ hint }}.</p>
                </Tooltip>
        ...
        <!-- Textarea -->
        <textarea v-else-if="inputType === 'textarea'" :value="modelValue as string | number"
            @input="$emit('update:modelValue', ($event.target as HTMLInputElement).value)" :placeholder="placeholder"
            :rows="rows" :class="['w-full rounded-lg border bg-transparent px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 shadow-theme-xs focus:outline-hidden focus:ring-3',
                error ? 'border-red-500 focus:ring-red-500/10' : 'border-gray-300 focus:border-brand-300 focus:ring-brand-500/10'
            ]"
            class="dark:bg-dark-900 text-gray-800 placeholder:text-gray-400 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
            :disabled="disabled" />
        ...
        <div v-else-if="inputType === 'file'" class="flex">
            <input ref="fileInput" type="file" @change="handleFileChange" :accept="accept" :multiple="multiple"
